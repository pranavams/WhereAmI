...
public abstract class CommandAvailabilityHelper {
	...
	/*
	 * All around this class, integers are used where unsigned bytes should be
	 * used. Too bad they don't exist in Java. Thank you Oracle.
	 */

	/**
	 * Digests the given string into an array of integers which can be used to
	 * check for command availability
	 * 
	 * @param availabilityString
	 *            An 8*n (where n is an integer) character string containing
	 *            only numbers and uppercase letters from A to F
	...
	public static int[] digestAvailabilityString(final String availabilityString) throws IllegalArgumentException {
		...
		// Each two characters of the string will be digested into one byte,
		// thus the resulting array will
	 ...
	 * returns the specified safetyReturn boolean instead of throwing and
	 * exception in the event of supplying an availabilityString which doesn't
	 * include information about the specified command
	 * 
	 * This is a direct call to {@link #isAvailable(String, int[], boolean)}
	...
	/**
	 * Checks whether the command identified by commandPid is available, as
	 * noted by availabilityString.
	 * 
	...
	public static boolean isAvailable(final String commandPid, final int[] availabilityArray) throws IllegalArgumentException {
		...
		int arrayIndex = (cmdNumber - 1) / 8; // the -1 corrects the command
												// code offset, as 00, 20, 40
												// are not the first commands in
												// each response to be evaluated
