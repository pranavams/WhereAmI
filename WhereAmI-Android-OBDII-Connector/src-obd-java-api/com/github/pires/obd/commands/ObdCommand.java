...
public abstract class ObdCommand {
	 ...
	 */
	private final Class[] ERROR_CLASSES = { UnableToConnectException.class, BusInitException.class, MisunderstoodCommandException.class,
			NoDataException.class, StoppedException.class, UnknownErrorException.class, UnsupportedCommandException.class };
	...
	public void run(InputStream in, OutputStream out) throws IOException, InterruptedException {
		synchronized (ObdCommand.class) {// Only one command can write and read
											// a data in one time.
	...
	protected void readRawData(InputStream in) throws IOException {
		 ...
		 * (trimmed above).
		 */
		// kills multiline.. rawData = rawData.substring(rawData.lastIndexOf(13)
		// + 1);
		rawData = rawData.replaceAll("\\s", "");// removes all [ \t\n\x0B\f\r]
	}
	 ...
	 * </p>
	 * 
	 * @return the command response in string representation, without
	 *         formatting.
	 */
